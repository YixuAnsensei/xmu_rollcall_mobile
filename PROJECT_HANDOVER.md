...
- **Android 端现状（已基本开发完成）**：
  - Android 端核心功能链路**已经大体完成并高度成熟**（版本 `v1.2.2`）：
    ...
    5. 32 个离线场景自动化回归测试（`bun test`，含 e2e 全链路降级仿真）全绿通过，TypeScript 类型检查 0 错误；
...
- **目录拓扑结构**：
  ...
  │   └── __tests__/
  │       ├── api.test.ts                 # 离线自动化测试套件（算法边界/请求形态）
  │       └── e2e.test.ts                 # 端到端测试套件（Bun.serve 仿真服务器，网络降级/拒绝场景）
  ├── modules/
...
### 5.1 离线测试套件 (`lib/__tests__/api.test.ts` + `lib/__tests__/e2e.test.ts`)
无需真实校园网络即可在本地快速验证全部算法边界与请求形态：
- **运行命令**：`bun test`（32 用例 / 116 断言；`e2e.test.ts` 以本地 Bun.serve 仿真服务器做端到端全链路验证）
- **用例矩阵覆盖**：
...
| **v1.2.1** | `ec418f9` | `a349e91e` | 深度对账修复两圆内切近似几何公式；建立 15 项全场景离线回归测试套件 | 归档可用 |
| **v1.2.2** | `2a2c3e1` | `b302659b` | 对齐桌面端网络降级（异常回落兜底分支）；补 kind 别名与 UTC+8 时间；全请求补超时；回归扩至 32 项 | 归档可用 |
| **v1.2.3** | （本次提交） | 待构建 | 课堂实测修复：数字签到活跃误判已结束（status 缺失回落记录自身 status）；发起/截止时间分列；csrftoken 误判会话 Cookie 修复；雷达日志上屏；回归 37 项 | **当前最新基准** |

> 注：v1.2.2 提交后当前工作区与远程 `origin/main` 一致。
